require('coffeescript/register')
import 'arr-ext'
const test = require('ava')

const {
  clearModel,
  createAdminAgent
} = require('../_test_envirment')

const _Category = require('../_category')
let Category

/* 準備環境 */
let ag = null
test.before('准备环境', async t => {
  ag = await createAdminAgent()
  Category = new _Category(ag.agent, ag.token)

  await clearModel()
})

test.serial('分类列表(空)', async t => {
  let web = await ag('get', '/api/categories').json(200)

  let data = web.json
  t.is(typeof data, 'object')
  t.true(Array.isArray(data.left))
  t.true(Array.isArray(data.right))

  t.is(data.left.length, 0)
  t.is(data.right.length, 0)
})

test.serial('分类列表', async t => {
  const left = await Category.create({
    name: '分类_left',
    position: 'left'
  })
  const right = await Category.create({
    name: '分类_right',
    position: 'right'
  })

  let web = await ag('get', '/api/categories').json(200)

  let data = web.json

  t.is(typeof data, 'object')
  t.true(Array.isArray(data.left))
  t.true(Array.isArray(data.right))

  t.truthy(data.left.findBy('_id', left._id))
  t.truthy(data.right.findBy('_id', right._id))
})

test('创建分类', async t => {
  let data = await Category.create({
    name: '分类1',
    position: 'right'
  })

  t.is(typeof data, 'object')
  t.is(data.name, '分类1')
  t.is(data.position, 'right')
})

test('不能创建重名的分类', async t => {
  await Category.create({ name: '重名' })

  const err = await Category.create({ name: '重名' }, 409)
  t.is(typeof err, 'object')
  t.truthy(err)
  t.is(typeof err.message, 'string')
})

test('删除分类', async t => {
  let web = await ag('post', '/api/category').testJson({
    name: '我会被删除的',
    position: 'left'
  }, 201)

  let created = web.json

  web = await ag('delete', `/api/category/${created._id}`).json(200)

  let deleted = web.json

  t.is(created._id, deleted._id)

  await ag('get', `/api/category/${deleted._id}`).json(404)
})

test('修改分类', async t => {
  const sourceValue = { name: 'source', color: '#BBBBBB' }
  const new_category = await Category.create(sourceValue)

  const web = await ag('patch', `/api/category/${new_category._id}`).testJson({
    name: '新名字',
    color: '#CCCCCC',
    position: 'right'
  }, 200)
  const mod_category = web.json

  t.is(mod_category.name, '新名字')
  t.is(mod_category.color, '#CCCCCC')
  t.is(mod_category.position, 'right')

  const category = await Category.get(mod_category._id)

  t.is(category.name, '新名字')
  t.is(category.color, '#CCCCCC')
  t.is(category.position, 'right')
})

test('修改分类(重复分类)', async t => {
  await Category.create({ name: 'duplicate' })

  const new_category = await Category.create({ name: 'other_category' })
  const web = await ag('patch', `/api/category/${new_category._id}`).testJson({
    name: 'duplicate'
  }, 409)

  let err = web.json
  t.is(typeof err, 'object')
  t.truthy(err)
  t.is(typeof err.message, 'string')
})
